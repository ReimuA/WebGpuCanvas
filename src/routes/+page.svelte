<script lang="ts">
	...
	import raymarchWarpingShader from '$lib/shaders/raymarch.warping.frag.wgsl';
	...
	let eye = { x: 0, y: 1, z: -10 };
	let viewplan = { x: 0.0, y: 0, z: 0.0 };
	...
	onMount(async () => {
		...
		const fragmentModule = device.createShaderModule({ code: raymarchWarpingShader });
